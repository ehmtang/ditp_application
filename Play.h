...
public:
	...
	// Returns true if the key is released from being held down
	// > https://docs.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
	bool KeyUp(int vKey);
...
DWORD timeGetTime()
{
	...
	auto durationMs = duration_cast<milliseconds>(currentTime.time_since_epoch());

	// Return the time in milliseconds as a DWORD (32-bit unsigned integer)
	return static_cast<DWORD>(durationMs.count());
}
